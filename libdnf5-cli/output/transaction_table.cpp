...
constexpr const char * PKG_HIGHLIGHT_COLOR = "lightblue";
...
TransactionTable::Impl::Impl(ITransaction & transaction) {
    ...
    auto column = scols_table_new_column(*tb, _("Package"), 0.2, 0);
    ...
    column = scols_table_new_column(*tb, _("Version"), 0.1, SCOLS_FL_TRUNC);
    header = scols_column_get_header(column);
    ...
    column = scols_table_new_column(*tb, _("Size"), 0.1, SCOLS_FL_RIGHT);
    header = scols_column_get_header(column);
    ...
    for (const auto & tspkg : tspkgs) {
        ...
        for (auto & replaced : tspkg->get_replaces()) {
            ...
            unsigned long small = (pkg->get_install_size() < replaced->get_install_size())
